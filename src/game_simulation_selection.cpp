...
SelectionResult GameSimulation::select_at(int32_t p_owner, const Vector2 &p_position) const {
	...
	for (const Base &base : bases) {
		if (base.owner == p_owner && distance_to(p_position, base.position) <= BASE_RADIUS + 18.0f) {
	...
	for (int32_t i = 0; i < static_cast<int32_t>(barracks.size()); ++i) {
		if (barracks[i].owner == p_owner && distance_to(p_position, barracks[i].position) <= BARRACKS_RADIUS + 18.0f) {
...
SelectionResult GameSimulation::select_units_in_rect(int32_t p_owner, const Rect2 &p_rect, bool p_has_unit_type_filter, UnitType p_unit_type_filter) const {
	...
	if (!p_has_unit_type_filter) {
		for (const Unit &unit : units) {
			if (unit.owner == p_owner && p_rect.has_point(unit.position)) {
	...
	for (const Unit &unit : units) {
		if (unit.owner == p_owner && p_rect.has_point(unit.position) && (!p_has_unit_type_filter || unit.type == p_unit_type_filter)) {
