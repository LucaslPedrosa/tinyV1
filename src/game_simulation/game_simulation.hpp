...
public:
	...
	int32_t get_gold(PlayerId p_owner) const;
	const ResourceWallet &get_resources(PlayerId p_owner) const;
	...
	bool has_building(PlayerId p_owner, BuildingType p_type) const;
	bool has_completed_building(PlayerId p_owner, BuildingType p_type) const;
	...
	bool can_start_building_placement(PlayerId p_owner, UnitId p_builder_unit_id,
	                                  BuildingType p_type) const;
	void get_available_actions(PlayerId p_owner, const std::vector<UnitId> &p_selected_unit_ids,
	                           BuildingId p_selected_building_id,
	                           std::vector<AvailableAction> &r_actions) const;
	bool does_overlap(const godot::Vector2 &p_position, const Footprint &p_footprint) const;
...
private:
	...
	Building *find_building(int32_t p_owner, BuildingType p_type);
	const Building *find_building(int32_t p_owner, BuildingType p_type) const;
	Building *find_building_by_id(BuildingId p_id);
	const Building *find_building_by_id(BuildingId p_id) const;
	Building *find_completed_building(int32_t p_owner, BuildingType p_type);
	BuildingId find_player_building_id_at(const godot::Vector2 &p_position) const;
	BuildingId find_enemy_building_id_at(int32_t p_owner, const godot::Vector2 &p_position) const;
	...
	bool enter_unit_combat(Unit &p_unit, UnitId p_target_unit_id);
	void damage_unit(Unit &p_attacker, Unit &p_target);
	...
	void move_unit_toward_with_avoidance(Unit &p_unit, const godot::Vector2 &p_target, float p_max_distance, ResourceId p_ignored_resource = -1, BuildingId p_ignored_building = -1);
	...
	BuildingId place_building(int32_t p_owner, BuildingType p_type, const godot::Vector2 &p_position, Unit *p_builder = nullptr);
	void delete_building(BuildingId p_building_id);
	bool set_selected_production_rally_point(int32_t p_owner, BuildingId p_selected_building_id, const godot::Vector2 &p_position);
	CommandFeedback command_selected_to(int32_t p_owner, const std::vector<int32_t> &p_selected_unit_ids, BuildingId p_selected_building_id, const godot::Vector2 &p_position);

	godot::Rect2 map_rect;
	std::vector<ResourceNode> resources;
	std::vector<Building> buildings;
	std::vector<Unit> units;
	ResourceWallet player_resources;
	ResourceWallet bot_resources;
