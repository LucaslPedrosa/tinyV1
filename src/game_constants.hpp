...
inline constexpr float RESOURCE_RADIUS = 28.0f;
inline constexpr float BASE_RADIUS = 42.0f;
inline constexpr float BARRACKS_RADIUS = 34.0f;
...
inline constexpr float BUILD_DISTANCE = 48.0f;
inline constexpr float GATHER_DISTANCE = 34.0f;
inline constexpr float RETURN_DISTANCE = 52.0f;
...
inline godot::Vector2 move_toward(const godot::Vector2 &from, const godot::Vector2 &to, float max_distance) {
...
inline godot::Vector2 formation_offset(int32_t p_index, int32_t p_count) {
	...
	return godot::Vector2(static_cast<float>(column) * spacing - width * 0.5f, static_cast<float>(row) * spacing - height * 0.5f);
}
